#!/usr/bin/env bash
# Generate vara.ini and varafm.ini for Digirig Lite (USB Audio Device) under a profile
# directory for tools like varanny to swap at runtime.
#
# Default profile dir: /opt/vara/profiles/digirig-lite
...
usage() {
  cat <<'USAGE'
Usage: create-vara-ini-digirig-lite.sh [-o DIR]

  -o DIR   Profile directory (default: /opt/vara/profiles/digirig-lite, or $VARA_PROFILE_DIR)

Writes vara.ini and varafm.ini with Digirig Lite sound device strings. Fills:
...
resolve_profile_dir() {
  if [[ -n "${OPT_OUT_DIR:-}" ]]; then
    ...
    printf '%s' "${VARA_ROOT}/profiles/digirig-lite"
  fi
}
